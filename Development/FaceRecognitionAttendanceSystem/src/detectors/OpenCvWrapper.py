...
def detect_face(detector, img, align = True):
	...
	faces = []
	try:
		#faces = detector["face_detector"].detectMultiScale(img, 1.3, 5) #option2:(img, 1.1, 10)
		faces = detector["face_detector"].detectMultiScale(img, scaleFactor=1.1, minNeighbors=10, minSize=(120,120)) # changing scaleFactor & minSize could affect speed & accuracy
	except:
		pass
...
def align_face(eye_detector, img):
	...
	#eyes = eye_detector.detectMultiScale(detected_face_gray, 1.3, 5) ##option2:(img, 1.1, 10)
	eyes = eye_detector.detectMultiScale(detected_face_gray, 1.1, 10) # Modify Here to change speed & accuracy
